...
import io.github.ngspace.hudder.v2runtime.values.V2Number;
import io.github.ngspace.hudder.v2runtime.values.V2Value;


/**
 * Modified version of HudderV2Compiler.java to spice up the syntax a bit.
 *   conditions are now done with / instead of %
 *   variables are now done with ( ) instead of { }
 *   methods are now done with + instead of ;
 *   if/while/def are now done with @ instead of #
 */
public class CustomCompiler extends AV2Compiler {
	...
	public V2Value getV2Value(String string) {
		if (string.equals("one"))
			return new V2Number(1, this);// The variable "one" will now return the number 1
		return super.getV2Value(string);
	}
